...
op.setExecute(async(client, err, userMsg, data) => {
    const { _id } = typeof err == "string" ? await client.data.errordb.insert({error: err || "No Error Message", timestamp: Date.now(), userMsg, data: {origin: data?.origin, messageSource: data?.messageSource?.url, content: data?.content}}) : await client.data.errordb.insert({error: util.inspect(err, {promise: false, depth: null}), timestamp: Date.now(), userMsg, data});;
