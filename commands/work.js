...
command.setExecute(async(client, message, args, cmd) => {
    ...
    await client.data.db.query(`update globalUser set lastWork=?, currencyUnit0=? where userID='${message.author.id}'`, [client.timeManager.timeToSqlDateTime(Date.now()), oldVal + val]);
    return message.channel.send(client.operations.generateEmbed.run({
