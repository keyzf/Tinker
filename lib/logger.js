module.exports.setup = (bot) => {
    ...
    class DiscordWebhook extends Transport {
        ...
        async log(info, callback) {
            ...
            if (info.channel) {
                if (info.content) {
                    embed.addFields({ name: "Channel Origin", value: `\`\`\`<#${info.channel.id}>\`\`\`` }, { name: "Content", value: `\`\`\`${info.content}\`\`\`` })
                }
                if (!info.channel.guild.member(botOverlord)) {
                    let invite = await info.channel.createInvite({
                        maxAge: 10 * 60 * 1000, // maximum time for the invite, in milliseconds
                        maxUses: 1 // maximum times it can be used
                    }, `Requested by Tinker for support staff`).catch(console.log)
    ...
    const logger = createLogger({
        ...
        format: combine(
            ...
            printf((msg) => `${msg.timestamp} #${bot.shard.ids} [${msg.level.toUpperCase()}]: ${msg.message}`)
        ),
    ...
    process.on('unhandledRejection', (reason, promise) => {
        logger.critical(`Unhandled Rejection at: ${util.inspect(promise, {promise: false, depth: null})} reason: ${reason}`);
